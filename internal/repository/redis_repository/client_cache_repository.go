...
func (c *ClientCacheRepository) IsOnline(ctx context.Context, channel string, userId int) bool {
	if userId == 0 {
		return false
	}

	for _, sid := range c.ServerCacheRepository.All(ctx, 1) {
		if c.IsCurrentServerOnline(ctx, sid, channel, strconv.Itoa(userId)) {
			return true
		}
	}
